...
mocha_1.describe('Client pool', () => {
    mocha_1.it('creates new instances as needed', () => {
    ...
    mocha_1.it('re-uses instances with remaining capacity', () => {
    ...
    mocha_1.it('re-uses idle instances', async () => {
        let instanceCount = 0;
    ...
    mocha_1.it('bin packs operations', async () => {
        let clientCount = 0;
        const clientPool = new pool_1.ClientPool(2, 0, () => {
            return ++clientCount;
        });
    ...
    mocha_1.it('garbage collects after success', () => {
        const clientPool = new pool_1.ClientPool(2, 0, () => {
            return {};
        });
        chai_1.expect(clientPool.size).to.equal(0);
    ...
    mocha_1.it('garbage collects after error', () => {
        const clientPool = new pool_1.ClientPool(2, 0, () => {
            return {};
        });
        chai_1.expect(clientPool.size).to.equal(0);
    ...
    mocha_1.it('garbage collection calls destructor', () => {
    ...
    mocha_1.it('keeps pool of idle clients', async () => {
    ...
    mocha_1.it('default setting keeps at least one idle client', async () => {
    ...
    mocha_1.it('rejects subsequent operations after being terminated', () => {
        const clientPool = new pool_1.ClientPool(1, 0, () => {
            return {};
        });
        return clientPool
            ...
            .catch((err) => {
            chai_1.expect(err.message).to.equal(pool_1.CLIENT_TERMINATED_ERROR_MSG);
        });
    });
    mocha_1.it('waits for existing operations to complete before releasing clients', done => {
        const clientPool = new pool_1.ClientPool(1, 0, () => {
            return {};
        });
        ...
        chai_1.expect(terminated).to.be.false;
        // Mark the mock operation as "complete".
        deferred.resolve();
        terminateOp.then(() => {
            chai_1.expect(terminated).to.be.true;
            done();
        });
    });
});
