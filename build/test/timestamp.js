...
mocha_1.describe('timestamps', () => {
    mocha_1.it('returned by default', () => {
    ...
    mocha_1.it('retain seconds and nanoseconds', () => {
        return createInstance({}, DOCUMENT_WITH_TIMESTAMP).then(firestore => {
            return firestore
                .doc('collectionId/documentId')
                .get()
    ...
    mocha_1.it('support missing values', () => {
    ...
    mocha_1.it('constructed using helper', () => {
    ...
    mocha_1.it('validates seconds', () => {
        chai_1.expect(() => new Firestore.Timestamp(0.1, 0)).to.throw('Value for argument "seconds" is not a valid integer.');
        chai_1.expect(() => new Firestore.Timestamp(-62135596801, 0)).to.throw('Value for argument "seconds" must be within [-62135596800, 253402300799] inclusive, but was: -62135596801');
        chai_1.expect(() => new Firestore.Timestamp(253402300800, 0)).to.throw('Value for argument "seconds" must be within [-62135596800, 253402300799] inclusive, but was: 253402300800');
    });
    mocha_1.it('validates nanoseconds', () => {
    ...
    mocha_1.it('valueOf', () => {
        chai_1.expect(new Firestore.Timestamp(-62135596677, 456).valueOf()).to.equal('000000000123.000000456');
        chai_1.expect(new Firestore.Timestamp(-62135596800, 0).valueOf()).to.equal('000000000000.000000000');
        chai_1.expect(new Firestore.Timestamp(253402300799, 1e9 - 1).valueOf()).to.equal('315537897599.999999999');
    });
    mocha_1.it('arithmetic comparison of a Timestamp object to itself', () => {
        const timestamp = new Firestore.Timestamp(1, 1);
        chai_1.expect(timestamp < timestamp).to.be.false;
        chai_1.expect(timestamp <= timestamp).to.be.true;
        chai_1.expect(timestamp > timestamp).to.be.false;
        chai_1.expect(timestamp >= timestamp).to.be.true;
    });
    mocha_1.it('arithmetic comparison of equivalent, but distinct, Timestamp objects', () => {
        const t1 = new Firestore.Timestamp(1, 1);
        const t2 = new Firestore.Timestamp(1, 1);
        chai_1.expect(t1 < t2).to.be.false;
        chai_1.expect(t1 <= t2).to.be.true;
        chai_1.expect(t1 > t2).to.be.false;
        chai_1.expect(t1 >= t2).to.be.true;
    });
    mocha_1.it('arithmetic comparison of Timestamp objects whose nanoseconds differ', () => {
        const t1 = new Firestore.Timestamp(1, 1);
    ...
    mocha_1.it('arithmetic comparison of Timestamp objects whose seconds differ', () => {
    ...
    mocha_1.it('arithmetic comparison of the smallest and largest Timestamp objects', () => {
