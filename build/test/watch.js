...
function docsEqual(actual, expected) {
    ...
    for (let i = 0; i < actual.length; i++) {
        chai_1.expect(actual[i].ref.id).to.equal(expected[i].ref.id);
        chai_1.expect(actual[i].data()).to.deep.eq(expected[i].data());
        chai_1.expect(expected[i].createTime).to.be.an.instanceOf(src_1.Timestamp);
        chai_1.expect(expected[i].updateTime).to.be.an.instanceOf(src_1.Timestamp);
    }
}
/**
...
function snapshotsEqual(lastSnapshot, version, actual, expected) {
    ...
    chai_1.expect(actual).to.be.an.instanceof(src_1.QuerySnapshot);
    ...
    for (let i = 0; i < expected.docChanges.length; i++) {
        ...
        chai_1.expect(actualDocChanges[i].doc.data()).to.deep.eq(expected.docChanges[i].doc.data());
        const readVersion = actualDocChanges[i].type === 'removed' ? version - 1 : version;
        chai_1.expect(actualDocChanges[i].doc.readTime.isEqual(new src_1.Timestamp(0, readVersion))).to.be.true;
    ...
    chai_1.expect(actualSnapshot.readTime.isEqual(new src_1.Timestamp(0, version))).to.be.true;
    chai_1.expect(actualSnapshot.size).to.equal(expected.docs.length);
...
class DeferredListener {
    ...
    on(type, data) {
        ...
        if (listener) {
            chai_1.expect(type).to.equal(listener.type, `Expected message of type '${listener.type}' but got '${type}' ` +
    ...
    await(expectedType) {
        ...
        if (data) {
            chai_1.expect(data.type).to.equal(expectedType, `Expected message of type '${expectedType}' but got '${data.type}' ` +
...
class WatchHelper {
    ...
    runFailedTest(expectedRequest, func, expectedError) {
        ...
        return this.streamHelper
            ...
            chai_1.expect(err.message).to.equal(expectedError);
        });
    }
}
mocha_1.describe('Query watch', () => {
    ...
    mocha_1.beforeEach(() => {
        ...
        backoff_1.setTimeoutHandler((op, timeout) => {
        ...
        return helpers_1.createInstance({ listen: () => listenCallback() }).then(firestoreClient => {
    ...
    mocha_1.afterEach(() => {
        ...
        return helpers_1.verifyInstance(firestore);
    });
    mocha_1.it('with invalid callbacks', () => {
        chai_1.expect(() => colRef.onSnapshot('foo')).to.throw('Value for argument "onNext" is not a valid function.');
        chai_1.expect(() => colRef.onSnapshot(() => { }, 'foo')).to.throw('Value for argument "onError" is not a valid function.');
    });
    mocha_1.it('without error callback', done => {
    ...
    mocha_1.it('handles invalid listen protos', () => {
    ...
    mocha_1.it('handles invalid target change protos', () => {
        return watchHelper.runFailedTest(collQueryJSON(), () => {
            // Mock the server responding to the query with an invalid proto.
            streamHelper.write({
                targetChange: {
    ...
    mocha_1.it('handles remove target change protos', () => {
        return watchHelper.runFailedTest(collQueryJSON(), () => {
            watchHelper.sendRemoveTarget();
        }, 'Error 13: internal error');
    });
    mocha_1.it('handles remove target change with code', () => {
        return watchHelper.runFailedTest(collQueryJSON(), () => {
            watchHelper.sendRemoveTarget(7);
        }, 'Error 7: test remove');
    });
    mocha_1.it('rejects an unknown target', () => {
        return watchHelper.runFailedTest(collQueryJSON(), () => {
            watchHelper.sendAddTarget(2);
        }, 'Unexpected target ID sent by server');
    });
    mocha_1.it('re-opens on unexpected stream end', () => {
        return watchHelper.runTest(collQueryJSON(), () => {
            ...
            return watchHelper.await('snapshot').then(async () => {
                ...
                await streamHelper.await('end');
                await streamHelper.awaitOpen();
                chai_1.expect(streamHelper.streamCount).to.equal(3);
            });
        });
    });
    mocha_1.it('stops attempts after maximum retry attempts', () => {
    ...
    mocha_1.it("doesn't re-open inactive stream", () => {
        ...
        return streamHelper
            ...
            streamHelper.close();
            unsubscribe();
            chai_1.expect(streamHelper.streamCount).to.equal(1);
        });
    });
    mocha_1.it('retries based on error code', async () => {
    ...
    mocha_1.it('retries with unknown code', () => {
        return watchHelper.runTest(collQueryJSON(), () => {
            watchHelper.sendAddTarget();
            watchHelper.sendCurrent();
            watchHelper.sendSnapshot(1, Buffer.from([0xabcd]));
    ...
    mocha_1.it('reconnects after error', () => {
    ...
    mocha_1.it('ignores changes sent after the last snapshot', () => {
        return watchHelper.runTest(collQueryJSON(), () => {
            // Mock the server responding to the query.
            watchHelper.sendAddTarget();
    ...
    mocha_1.it('ignores non-matching tokens', () => {
        return watchHelper.runTest(collQueryJSON(), () => {
            // Mock the server responding to the query.
            watchHelper.sendAddTarget();
            watchHelper.sendCurrent();
            ...
            return watchHelper
                ...
                .then(request => {
                verifyRequest(request, resumeTokenQuery(resumeToken));
                chai_1.expect(streamHelper.streamCount).to.equal(2);
            });
        });
    });
    mocha_1.it('reconnects with multiple attempts', () => {
        return watchHelper
            ...
            .then(() => {
            chai_1.expect(streamHelper.streamCount).to.equal(6, 'Expected stream to be opened once and retried five times');
        });
    });
    mocha_1.it('sorts docs', () => {
    ...
    mocha_1.it('combines multiple change events for the same doc', () => {
        return watchHelper.runTest(collQueryJSON(), () => {
            // Mock the server responding to the query.
            watchHelper.sendAddTarget();
            // Add a result.
    ...
    mocha_1.it('can sort by FieldPath.documentId()', () => {
    ...
    mocha_1.it('sorts document changes in the right order', () => {
        return watchHelper.runTest(collQueryJSON(), () => {
            // Mock the server responding to the query.
            watchHelper.sendAddTarget();
            watchHelper.sendDoc(doc1, { foo: 'a' });
    ...
    mocha_1.it('handles removing a doc', () => {
        return watchHelper.runTest(collQueryJSON(), () => {
            // Mock the server responding to the query.
            watchHelper.sendAddTarget();
            watchHelper.sendCurrent();
    ...
    mocha_1.it('handles deleting a non-existent doc', () => {
        return watchHelper.runTest(collQueryJSON(), () => {
            // Mock the server responding to the query.
            watchHelper.sendAddTarget();
            watchHelper.sendCurrent();
    ...
    mocha_1.it('reset after idle timeout', async () => {
        let idleTimeout = () => { };
        backoff_1.setTimeoutHandler((op, timeout) => {
            if (timeout === watch_1.WATCH_IDLE_TIMEOUT_MS) {
                idleTimeout = op;
    ...
    mocha_1.it('handles reset with phantom doc', () => {
        return watchHelper.runTest(collQueryJSON(), () => {
            // Mock the server responding to the query.
            watchHelper.sendAddTarget();
            watchHelper.sendCurrent();
    ...
    mocha_1.it('handles sending the snapshot version multiple times', () => {
        return watchHelper.runTest(collQueryJSON(), () => {
            // Mock the server responding to the query.
            watchHelper.sendAddTarget();
            watchHelper.sendCurrent();
    ...
    mocha_1.it('handles filter mismatch', () => {
        ...
        return watchHelper.runTest(collQueryJSON(), () => {
            ...
            return watchHelper
                ...
                .then(request => {
                chai_1.expect(streamHelper.streamCount).to.equal(2);
                chai_1.expect(oldRequestStream).to.not.equal(streamHelper.writeStream);
                verifyRequest(request, collQueryJSON());
                watchHelper.sendAddTarget();
                watchHelper.sendCurrent();
                watchHelper.sendSnapshot(3);
    ...
    mocha_1.it('handles filter match', () => {
        return watchHelper.runTest(collQueryJSON(), () => {
            watchHelper.sendAddTarget();
            // Add a result.
            watchHelper.sendDoc(doc1, { foo: 'a' });
    ...
    mocha_1.it('handles resets with pending updates', () => {
        return watchHelper.runTest(collQueryJSON(), () => {
            watchHelper.sendAddTarget();
            // Add a result.
            watchHelper.sendDoc(doc1, { foo: 'a' });
    ...
    mocha_1.it('handles add and delete in same snapshot', () => {
        return watchHelper.runTest(collQueryJSON(), () => {
            // Mock the server responding to the query.
            watchHelper.sendAddTarget();
            watchHelper.sendCurrent();
    ...
    mocha_1.it('handles non-changing modify', () => {
        return watchHelper.runTest(collQueryJSON(), () => {
            // Mock the server responding to the query.
            watchHelper.sendAddTarget();
            watchHelper.sendCurrent();
    ...
    mocha_1.it('handles update time change', () => {
        return watchHelper.runTest(collQueryJSON(), () => {
            // Mock the server responding to the query.
            watchHelper.sendAddTarget();
            watchHelper.sendCurrent();
    ...
    mocha_1.describe('supports isEqual', () => {
        let snapshotVersion;
        mocha_1.beforeEach(() => {
            snapshotVersion = 0;
        });
        ...
        mocha_1.it('for equal snapshots', () => {
            let firstSnapshot;
            let secondSnapshot;
            let thirdSnapshot;
            return initialSnapshot(snapshot => {
                return nextSnapshot(snapshot, snapshot => {
                    ...
                    chai_1.expect(firstSnapshot.isEqual(firstSnapshot)).to.be.true;
                    watchHelper.sendDoc(doc1, { foo: 'a' });
                    watchHelper.sendDoc(doc2, { foo: 'b' });
                    watchHelper.sendDoc(doc3, { foo: 'c' });
                })
                    ...
                    chai_1.expect(secondSnapshot.isEqual(secondSnapshot)).to.be.true;
                    watchHelper.sendDocDelete(doc1);
                    ...
                    chai_1.expect(thirdSnapshot.isEqual(thirdSnapshot)).to.be.true;
                });
            }).then(() => initialSnapshot(snapshot => {
                return nextSnapshot(snapshot, snapshot => {
                    chai_1.expect(snapshot.isEqual(firstSnapshot)).to.be.true;
                    watchHelper.sendDoc(doc1, { foo: 'a' });
                    watchHelper.sendDoc(doc2, { foo: 'b' });
                    watchHelper.sendDoc(doc3, { foo: 'c' });
                })
                    .then(snapshot => nextSnapshot(snapshot, snapshot => {
                    chai_1.expect(snapshot.isEqual(secondSnapshot)).to.be.true;
                    watchHelper.sendDocDelete(doc1);
                    watchHelper.sendDoc(doc2, { foo: 'bar' });
                    watchHelper.sendDoc(doc4, { foo: 'd' });
                }))
                    .then(snapshot => {
                    chai_1.expect(snapshot.isEqual(thirdSnapshot)).to.be.true;
                });
            }));
        });
        mocha_1.it('for equal snapshots with materialized changes', () => {
            let firstSnapshot;
            return initialSnapshot(snapshot => {
                ...
                return nextSnapshot(snapshot, () => {
                    ...
                    chai_1.expect(materializedDocs.length).to.equal(3);
                    chai_1.expect(snapshot.isEqual(firstSnapshot)).to.be.true;
                });
            }));
        });
        mocha_1.it('for snapshots of different size', () => {
            let firstSnapshot;
            return initialSnapshot(snapshot => {
                return nextSnapshot(snapshot, () => {
                    watchHelper.sendDoc(doc1, { foo: 'a' });
                ...
                return nextSnapshot(snapshot, () => {
                    ...
                    chai_1.expect(snapshot.isEqual(firstSnapshot)).to.be.false;
                });
            }));
        });
        mocha_1.it('for snapshots with different kind of changes', () => {
            let firstSnapshot;
            return initialSnapshot(snapshot => {
                return nextSnapshot(snapshot, () => {
                    watchHelper.sendDoc(doc1, { foo: 'a' });
                    ...
                    firstSnapshot = snapshot;
                    chai_1.expect(snapshot.docChanges()[0].isEqual(firstSnapshot.docChanges()[0])).to.be.true;
                });
            }).then(() => initialSnapshot(snapshot => {
                return nextSnapshot(snapshot, () => {
                    watchHelper.sendDoc(doc1, { foo: 'b' });
                    ...
                    chai_1.expect(snapshot.isEqual(firstSnapshot)).to.be.false;
                    chai_1.expect(snapshot.docChanges()[0].isEqual(firstSnapshot.docChanges()[0])).to.be.false;
                });
            }));
        });
        mocha_1.it('for snapshots with different number of changes', () => {
            let firstSnapshot;
            return initialSnapshot(snapshot => {
        ...
        mocha_1.it('for snapshots with different data types', () => {
            ...
            return initialSnapshot(snapshot => {
                ...
                return nextSnapshot(snapshot, () => watchHelper.sendDoc(doc1, { foo: 1 })).then(snapshot => {
                    chai_1.expect(snapshot.isEqual(originalSnapshot)).to.be.false;
                });
            }));
        });
        mocha_1.it('for snapshots with different queries', () => {
            let firstSnapshot;
            return initialSnapshot(snapshot => {
                firstSnapshot = snapshot;
            }).then(() => {
        ...
        mocha_1.it('for objects with different type', () => {
            return initialSnapshot(snapshot => {
                chai_1.expect(snapshot.isEqual('foo')).to.be.false;
                chai_1.expect(snapshot.isEqual({})).to.be.false;
                chai_1.expect(snapshot.isEqual(new src_1.GeoPoint(0, 0))).to.be
                    .false;
            });
        });
    });
    mocha_1.it('handles delete and re-add in same snapshot', () => {
        return watchHelper.runTest(collQueryJSON(), () => {
            // Mock the server responding to the query.
            watchHelper.sendAddTarget();
            watchHelper.sendCurrent();
...
mocha_1.describe('DocumentReference watch', () => {
    ...
    mocha_1.it('with invalid callbacks', () => {
        chai_1.expect(() => doc.onSnapshot('foo')).to.throw('Value for argument "onNext" is not a valid function.');
        chai_1.expect(() => doc.onSnapshot(() => { }, 'foo')).to.throw('Value for argument "onError" is not a valid function.');
    });
    mocha_1.it('without error callback', done => {
    ...
    mocha_1.it('handles changing a doc', () => {
        return watchHelper.runTest(watchJSON(), () => {
            ...
            return watchHelper
                ...
                chai_1.expect(snapshot.createTime.isEqual(new src_1.Timestamp(1, 2))).to.be.true;
                chai_1.expect(snapshot.updateTime.isEqual(new src_1.Timestamp(3, 1))).to.be.true;
    ...
    mocha_1.it('ignores non-matching doc', () => {
        return watchHelper.runTest(watchJSON(), () => {
            // Mock the server responding to the watch.
            watchHelper.sendAddTarget();
            watchHelper.sendCurrent();
    ...
    mocha_1.it('combines multiple change events for the same doc', () => {
        return watchHelper.runTest(watchJSON(), () => {
            ...
            return watchHelper
                ...
                .then(snapshot => {
                chai_1.expect(snapshot.get('foo')).to.equal('d');
                // Remove it, delete it, and then add it again.
                ...
                .then(snapshot => {
                chai_1.expect(snapshot.get('foo')).to.equal('f');
            });
        });
    });
    mocha_1.it('handles deleting a doc', () => {
...
mocha_1.describe('Query comparator', () => {
    let firestore;
    let colRef;
    let doc1;
    let doc2;
    ...
    mocha_1.it('handles descending case', () => {
    ...
    mocha_1.it('handles nested fields', () => {
    ...
    mocha_1.it('fails on missing fields', () => {
        ...
        const comparator = query.comparator();
        chai_1.expect(() => input.sort(comparator)).to.throw("Trying to compare documents on fields that don't exist");
    });
});
