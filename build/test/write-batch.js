...
mocha_1.describe('set() method', () => {
    ...
    mocha_1.it('accepts document data', () => {
        writeBatch.set(firestore.doc('sub/doc'), { foo: 'bar' });
    });
    mocha_1.it('works with null objects', () => {
        const nullObject = Object.create(null);
        nullObject.bar = 'ack';
        writeBatch.set(firestore.doc('sub/doc'), nullObject);
    });
    mocha_1.it('requires the correct converter for Partial usage', async () => {
        const converter = {
            toFirestore(post) {
        ...
        chai_1.expect(() => writeBatch.set(ref, { title: 'foo' }, { merge: true })).to.throw('Value for argument "data" is not a valid Firestore document. Cannot use "undefined" as a Firestore value (found in field "author").');
    });
});
mocha_1.describe('delete() method', () => {
    let firestore;
    let writeBatch;
    mocha_1.beforeEach(() => {
...
mocha_1.describe('create() method', () => {
    let firestore;
    let writeBatch;
    mocha_1.beforeEach(() => {
        return helpers_1.createInstance().then(firestoreClient => {
...
mocha_1.describe('batch support', () => {
    ...
    mocha_1.it('accepts multiple operations', () => {
    ...
    mocha_1.it('chains multiple operations', () => {
        const documentName = firestore.doc('col/doc');
        return writeBatch
    ...
    mocha_1.it('cannot append to committed batch', () => {
        const documentName = firestore.doc('col/doc');
    ...
    mocha_1.it('can reset a committed batch', async () => {
        const documentName = firestore.doc('col/doc');
        const batch = firestore.batch();
        batch.set(documentName, { foo: src_1.FieldValue.serverTimestamp() });
        batch.update(documentName, { foo: 'bar' });
    ...
    mocha_1.it('can commit an unmodified batch multiple times', () => {
        const documentName = firestore.doc('col/doc');
        const batch = firestore.batch();
        batch.set(documentName, { foo: src_1.FieldValue.serverTimestamp() });
        batch.update(documentName, { foo: 'bar' });
    ...
    mocha_1.it('can return same write result', () => {
        const overrides = {
            commit: () => {
                return helpers_1.response({
                    commitTime: {
...
mocha_1.describe('bulkCommit support', () => {
    const documentName = `projects/${PROJECT_ID}/databases/(default)/documents/col/doc`;
    let firestore;
    let writeBatch;
    mocha_1.beforeEach(() => {
    ...
    function verifyResponse(writeResults) {
        chai_1.expect(writeResults[0].writeTime.isEqual(new src_1.Timestamp(0, 0))).to.be.true;
        chai_1.expect(writeResults[1].writeTime).to.be.null;
        chai_1.expect(writeResults[0].status.code).to.equal(google_gax_1.Status.OK);
        chai_1.expect(writeResults[1].status.code).to.equal(google_gax_1.Status.DEADLINE_EXCEEDED);
    }
    mocha_1.it('bulkCommit', () => {
        const documentName = firestore.doc('col/doc');
        writeBatch.set(documentName, { foo: src_1.FieldValue.serverTimestamp() });
        writeBatch.update(documentName, { foo: 'bar' });
